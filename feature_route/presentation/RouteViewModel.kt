...
import com.polygonbikes.ebike.core.route.FriendsRouteRoute
...
class RouteViewModel @Inject constructor(
    ...
    private var _state: MutableState<RouteState> = mutableStateOf(RouteState())
    ...
    private fun getRouteData() {
        val call: Call<ListRouteResponseMiddleware> = routeServiceMiddleware.getListRoute(recommended = true)
        ...
        call.enqueue(object : Callback<ListRouteResponseMiddleware> {
            override fun onResponse(
                ...
                _state.value = state.value.copy(isRecommendedRouteLoading = false)
    ...
    fun onEvent(event: RouteEvent) {
        when (event) {
            ...
            RouteEvent.OpenFriendsRoute -> {
                sendUiEvent(
                    Navigate(
                        route = FriendsRouteRoute
                    )
                )
            }
        }
    ...
    private fun fetchFriendsRoutes() {
        _state.value = state.value.copy(isFriendsRouteLoading = true)
        ...
        routeCall.enqueue(object : Callback<ListRouteResponseMiddleware> {
            override fun onResponse(
                ...
                _state.value = state.value.copy(isFriendsRouteLoading = false)
            }
    ...
    private fun fetchRoutes(filter: RouteFilterData) {
        _state.value = state.value.copy(isRecommendedRouteLoading = true)
